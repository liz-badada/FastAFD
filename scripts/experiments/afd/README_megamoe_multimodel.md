...
2. Run the exact workload grid needed by the AIC sweep. All grid variables are
   ...
   The fixed-pool AFD search can concentrate much more work onto each F rank
   than the 48/96 source-rank qualification grid. Run the checked-in case
   matrix before fitting a load-based timing model for those larger A:F
   ratios:

   ```bash
   SOURCE_ROOT=/path/to/FastAFD \
   RUN_SCRIPT=run_megamoe_m2n_model_case_matrix.sh \
   CASE_MATRIX=/workspace/FastAFD/scripts/experiments/afd/reference/b200_sxm/high_load_case_matrix.txt \
   RESULTS_DIR=/workspace/results/megamoe-high-load \
   WARMUPS=30 ITERATIONS=40 \
   sbatch scripts/experiments/afd/submit_megamoe_m2n_b200.sbatch
   ```

   Each row is `model split sequences_per_A_rank mtp_nextn microbatches
   backend`. The matrix covers the per-F-rank load envelope reached by the
   16/24/36/48/72-GPU generic-control winners. These remain B200
...
3. Convert raw JSON files into the review table and exact-only AIC profile.
   ...
   DeepEP+DeepGEMM, plus 120 split-stage measurements for each backend. All 320
   backend-qualified entries pass their applicable gates. Raw per-run JSON is
   intentionally not committed and is regenerated by the commands above at each
   entry's `source.result` path. This profile is labeled `b200_sxm` and is
   intentionally rejected by an exact-match GB200 sweep.
